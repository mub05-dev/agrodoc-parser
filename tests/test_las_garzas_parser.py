...
def parser():
    return LasGarzasParser()


@pytest.fixture(scope="module")
def result(parser, pdf_las_garzas):
    return parser.parse(str(pdf_las_garzas))


# --- detect ---

def test_detect_true(parser):
    assert parser.detect("LABORATORIO AGROPECUARIO LAS GARZAS INFORME") is True


def test_detect_case_insensitive(parser):
    assert parser.detect("laboratorio agropecuario las garzas") is True
...
def test_laboratory_key(parser):
    assert parser.laboratory_key == "las_garzas"
...
def test_parse_report_laboratory(result):
    assert "garzas" in result.report.laboratory.lower()
