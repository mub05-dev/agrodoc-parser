...
def parser():
    return PhytomonitorParser()


@pytest.fixture(scope="module")
def result(parser, pdf_phytomonitor):
    return parser.parse(str(pdf_phytomonitor))


# --- detect ---

def test_detect_true(parser):
    assert parser.detect("PHYTOMONITOR INFORME DE ANALISIS") is True


def test_detect_case_insensitive(parser):
    assert parser.detect("phytomonitor analisis") is True
...
def test_laboratory_key(parser):
    assert parser.laboratory_key == "phytomonitor"
...
def test_parse_report_laboratory(result):
    assert "phytomonitor" in result.report.laboratory.lower()
