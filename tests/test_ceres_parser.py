...
@pytest.fixture(scope="module")
def result(parser, pdf_ceres):
    return parser.parse(str(pdf_ceres))


# --- detect ---

def test_detect_true_ceres(parser):
    assert parser.detect("AGROLABORATORIO CERES INFORME") is True


def test_detect_true_labceres(parser):
    assert parser.detect("LABCERESGUATE RESULTADOS") is True


def test_detect_case_insensitive(parser):
    assert parser.detect("agrolaboratorio ceres") is True
...
def test_laboratory_key(parser):
    assert parser.laboratory_key == "ceres"
...
def test_parse_report_laboratory(result):
    assert "ceres" in result.report.laboratory.lower()
